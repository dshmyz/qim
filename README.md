...
# 编辑配置（根据需要修改数据库配置）
# 默认使用 SQLite，如需使用 MySQL 修改 database.type 为 mysql
```
...
**数据库说明：**
- 默认使用 SQLite，数据存储在 `qim.db` 文件
- 如需使用 MySQL，修改 `config.yaml` 中的 `database.type` 为 `mysql`，并配置数据库连接信息
...
SQLite 数据库会在首次运行时自动创建，无需手动执行建表语句。

***

## 🔄 数据迁移

如需从旧版 OIM 系统迁移数据，请参考：[数据迁移方案](./docs/realtime-communication/data-migration-from-schema.md)
