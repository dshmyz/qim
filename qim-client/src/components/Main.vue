...
const processConversation = (conv: any) => {
  const members = conv.members ? conv.members.map((member: any) => ({
    id: member.user && member.user.id ? member.user.id.toString() : (member.UserID ? member.UserID.toString() : ''),
  ...
  if (conversationIndex !== -1) {
    ...
    if (!memberExists) {
      ...
      const updatedMembers = [...(conversation.members || []), {
        ...
        name: newMember.nickname || newMember.username || (newMember.name !== undefined ? newMember.name : '未知用户'),
        avatar: newMember.avatar || ''
      }]
      console.log('更新后的群聊成员:', updatedMembers, conversation.members)
      // 创建新的会话对象，确保响应式更新
      const updatedConversation = {
        ...conversation,
        members: updatedMembers
...
const handleSendMessage = async (messageData: any) => {
  ...
  // 检查是否为模拟会话
  if (currentConversationId.value.startsWith('conv_')) {
  ...
  try {
    ...
    const response = await request(`/api/v1/conversations/${currentConversationId.value}/messages`, {
      method: 'POST',
