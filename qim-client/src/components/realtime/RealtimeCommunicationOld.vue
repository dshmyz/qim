...
const handleWebRTCOfferGlobal = (data: any) => {
  ...
  console.log('RealtimeCommunication: data.share_type:', data.share_type)
  console.log('RealtimeCommunication: data.call_type:', data.call_type)
  console.log('RealtimeCommunication: data.media_type:', data.media_type)
  ...
  if (data.share_type === 'screen' || data.media_type === 'screen') {
  ...
  } else if (data.call_type || data.media_type === 'video' || data.media_type === 'audio') {
    // 语音/视频通话
    ...
    console.warn('RealtimeCommunication: 将默认当作屏幕共享处理')
    handleWebRTCOffer(data)
  }
}
