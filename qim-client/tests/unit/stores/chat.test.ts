...
describe('useChatStore', () => {
  ...
  describe('业务逻辑方法', () => {
    describe('pinConversation', () => {
      it('应该能置顶会话（使用实现字段 is_pinned）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', is_pinned: false } as Conversation
        ])

        store.pinConversation('1', true)

        expect(store.conversations[0].is_pinned).toBe(true)
        expect(store.conversations[0].pinnedAt).toBeDefined()
      })

      it('应该能取消置顶会话（使用实现字段 is_pinned）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', is_pinned: true, pinnedAt: 123456 } as Conversation
        ])

        store.pinConversation('1', false)

        expect(store.conversations[0].is_pinned).toBe(false)
        expect(store.conversations[0].pinnedAt).toBeUndefined()
      })

      it('置顶后应该保存到 localStorage（兼容节流：推进定时器）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', is_pinned: false } as Conversation
        ])
        ...
        expect(stored[0]?.is_pinned).toBe(true)
      })
    })
    ...
    describe('receiveMessage', () => {
      ...
      it('非当前会话接收消息时应该增加未读计数（使用实现字段 unread_count）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', unread_count: 0 } as Conversation
        ])
    ...
    describe('markConversationRead', () => {
      it('应该能标记会话为已读（使用实现字段 unread_count）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', unread_count: 5 } as Conversation
        ])
    ...
    describe('sortedConversations', () => {
      it('置顶会话应该排在前面（使用实现字段 is_pinned）', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', is_pinned: false, timestamp: 3000 } as Conversation,
          { id: '2', name: '会话2', is_pinned: true, timestamp: 1000 } as Conversation,
          { id: '3', name: '会话3', is_pinned: false, timestamp: 2000 } as Conversation
        ])
      ...
      it('非置顶会话应该按时间戳降序排列', () => {
        const store = useChatStore()
        store.setConversations([
          { id: '1', name: '会话1', is_pinned: false, timestamp: 1000 } as Conversation,
          { id: '2', name: '会话2', is_pinned: false, timestamp: 3000 } as Conversation,
          { id: '3', name: '会话3', is_pinned: false, timestamp: 2000 } as Conversation
        ])
