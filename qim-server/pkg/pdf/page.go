...
func (f Font) getEncoder() TextEncoding {
	// ToUnicode CMap 是 PDF 规范规定的「字符码→Unicode」权威映射。
	// 只要字体带 ToUnicode（无论 /Encoding 是具名编码、Differences 字典还是缺省），
	// 一律优先使用；否则按 Encoding 种类回退。
	if f.V.Key("ToUnicode").Kind() == Stream {
	...
	case Name:
		...
		default:
			// GBK 系具名编码（GBK-EUC-H 等，知网/万方论文 Type0 字体常见）：
			// 无 ToUnicode 时内容流字符码即原始 GBK 字节，按 GBK 解码。
			if isGBKNamedEncoding(enc.Name()) {
				return &gbkEncoder{}
			}
			// 其余具名编码（Identity-H 等 Type0/CID 字体）：退 PDFDocEncoding 兜底，
			// 比直接输出裸字节（被上层 range 逐字节转成 U+FFFD 乱码）语义更好。
			if DebugOn {
