...
func TestValidateExternalURL(t *testing.T) {
	tests := []struct {
		...
		{"单零 0", "http://0:8080/mcp", true, "不允许访问本机"},
		{"双零 0.0", "http://0.0:8080/mcp", true, "不允许访问本机"},
		{"三零 0.0.0", "http://0.0.0:8080/mcp", true, "不允许访问本机"},

		// — 已知内部元数据/内网域名 —
		{"AWS 元数据", "http://169.254.169.254/latest/meta-data/", true, "不允许访问内网"},

		// — 输入校验 —
		{"空 URL", "", true, "仅支持 http/https"},
		{"无主机", "http://", true, "URL 缺少主机名"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := service.ValidateExternalURL(tt.url)
			if tt.wantErr {
				assert.Error(t, err)
				if tt.errMsg != "" {
