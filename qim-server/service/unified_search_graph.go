...
func (g *UnifiedSearchGraph) Build() error {
	...
	runnable, err := compileGraph(graph, "UnifiedSearch")
	if err != nil {
...
func (g *UnifiedSearchGraph) retrieve(ctx context.Context, input *UnifiedSearchInput) (*retrieveResult, error) {
...
func (g *UnifiedSearchGraph) buildPrompt(ctx context.Context, result *retrieveResult) ([]*schema.Message, error) {
...
- memory: 用户长期记忆`
	...
	if len(result.sources) > 0 {
		...
		sb.WriteString("【检索到的相关信息】\n\n")
		for i, src := range result.sources {
			sb.WriteString(fmt.Sprintf("--- 来源 %d: %s", i+1, getSourceTypeLabel(src.Type)))
			if src.Title != "" {
				sb.WriteString(fmt.Sprintf(" (%s)", src.Title))
			}
			if src.Sender != "" {
				sb.WriteString(fmt.Sprintf(" [发送者: %s]", src.Sender))
			}
			if src.Date != "" {
				sb.WriteString(fmt.Sprintf(" [时间: %s]", src.Date))
			}
			sb.WriteString(fmt.Sprintf(" [相关度: %.2f] ---\n", src.Relevance))
			sb.WriteString(src.Content)
			sb.WriteString("\n\n")
		}
		messages = append(messages, &schema.Message{Role: schema.User, Content: sb.String()})
		messages = append(messages, &schema.Message{Role: schema.Assistant, Content: "我已了解检索到的相关信息，请继续提问。"})
	} else {
		messages = append(messages, &schema.Message{Role: schema.User, Content: "没有检索到相关信息。"})
		messages = append(messages, &schema.Message{Role: schema.Assistant, Content: "了解，我将基于通用知识回答。"})
	}

	messages = append(messages, &schema.Message{Role: schema.User, Content: fmt.Sprintf("请回答：%s", result.query)})
	return messages, nil
}

func (g *UnifiedSearchGraph) format(ctx context.Context, msg *schema.Message) (*UnifiedSearchOutput, error) {
	return &UnifiedSearchOutput{Answer: msg.Content}, nil
}
