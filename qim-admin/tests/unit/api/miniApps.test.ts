...
describe('miniApps API', () => {
  ...
  describe('createMiniApp', () => {
    ...
    it('应该支持创建时不带可选参数', async () => {
      ...
      await createMiniApp(createData)
  ...
  describe('updateMiniApp', () => {
    ...
    it('应该支持只更新状态', async () => {
      const updatedMiniApp = { ...mockMiniApp, status: 'inactive' as const }
      const mockResponse = {
        data: {
          code: 0,
          message: 'success',
      ...
      await updateMiniApp(1, updateData)
    ...
    it('应该使用正确的 ID 构建 URL', async () => {
      ...
      await updateMiniApp(42, { name: 'test' })
