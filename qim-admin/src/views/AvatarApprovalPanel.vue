<template>
  <div class="avatar-approval-panel">
    ...
    <div class="action-bar">
      ...
      <el-button type="primary" @click="showEnableDialog">
    ...
    <el-table :data="approvals" v-loading="loading" style="width: 100%">
      <el-table-column label="申请人" min-width="140">
        <template #default="{ row }">
          <div class="user-cell">
            <el-avatar :size="28" :src="row.user_avatar">{{ row.user_name?.charAt(0) }}</el-avatar>
            <span>{{ row.user_name || '未知' }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="name" label="分身名称" min-width="120" />
      ...
      <el-table-column label="申请时间" width="180">
        <template #default="{ row }">
          {{ row.applied_at ? formatTime(row.applied_at) : '-' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          ...
          <span v-else-if="row.approval_status === 'rejected'" class="reviewed-info">
            已处理
          </span>
...
import { getAvatarApprovals, approveAvatar, rejectAvatar, enableAvatar, type AvatarApproval } from '@/api/avatarApprovals'
...
const approvals = ref<AvatarApproval[]>([])
...
const fetchApprovals = async () => {
  loading.value = true
  try {
    const { data } = await getAvatarApprovals({
    ...
    if (filterStatus.value !== 'pending') {
      const { data: pendingData } = await getAvatarApprovals({ status: 'pending', page: 1, pageSize: 1 })
...
const handleApprove = async (row: AvatarApproval) => {
  try {
    await ElMessageBox.confirm(`确定通过「${row.user_name}」的分身启用申请吗？`, '确认通过')
    await approveAvatar(row.id)
...
const handleReject = (row: AvatarApproval) => {
...
const confirmReject = async () => {
  ...
  try {
    await rejectAvatar(rejectingId.value, { reason: rejectReason.value.trim() })
...
const statusTagType = (status: string) => {
  const map: Record<string, string> = { pending: 'warning', approved: 'success', rejected: 'danger', none: 'info' }
  return map[status] || 'info'
}

const statusLabel = (status: string) => {
  const map: Record<string, string> = { pending: '待审批', approved: '已通过', rejected: '已拒绝', none: '未申请' }
