...
**核心功能**:
- 提供商列表（OpenAI、Anthropic、阿里云、百度、字节跳动等）
- API Key 配置
- API 端点配置
- 提供商启用/禁用
- 连接测试
...
**界面设计**:
- 提供商列表：显示各提供商的状态、配置信息
- 配置表单：填写 API Key、端点、其他参数
- 测试按钮：测试连接是否正常
- 状态指示器：显示连接状态
...
#### AI 模型配置 API
- `GET /api/ai/providers` - 获取模型提供商列表
- `POST /api/ai/providers` - 添加模型提供商
- `PUT /api/ai/providers/:id` - 更新模型提供商配置
- `DELETE /api/ai/providers/:id` - 删除模型提供商
- `POST /api/ai/providers/:id/test` - 测试模型提供商连接
- `GET /api/ai/config` - 获取模型参数配置
- `PUT /api/ai/config` - 更新模型参数配置
- `GET /api/ai/quota` - 获取配额配置
- `PUT /api/ai/quota` - 更新配额配置
- `GET /api/ai/statistics` - 获取使用统计
- `GET /api/ai/statistics/usage` - 获取详细使用记录
...
CREATE TABLE ai_provider (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  name VARCHAR(50) NOT NULL COMMENT 'openai, anthropic, alibaba, baidu, bytedance',
  ...
  enabled BOOLEAN DEFAULT TRUE,
  status VARCHAR(20) DEFAULT 'unknown' COMMENT 'unknown, connected, error',
...
CREATE TABLE ai_quota (
  ...
  daily_limit INT DEFAULT 100 COMMENT '每日调用次数限制',
  token_limit INT DEFAULT 10000 COMMENT 'Token 消耗限制',
  concurrent_limit INT DEFAULT 5 COMMENT '并发请求数限制',
  overlimit_strategy VARCHAR(20) DEFAULT 'reject' COMMENT 'reject, degrade, notify',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE ai_usage (
  ...
  status VARCHAR(20) COMMENT 'success, error, timeout',
  error_message TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_user_id (user_id),
